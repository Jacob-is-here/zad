...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnAllSalesmen()
    {
        ...
        List<Emp> result = emps.Where(s=> s.Job == "SALESMAN").ToList(); 
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(s=> s.DeptNo == 30).OrderByDescending(s =>s.Sal).ToList(); 
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(s => depts.Where(d=>d.Loc == "CHICAGO").Select(g => g.DeptNo).Contains(s.DeptNo)).ToList(); 
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(s => s.DeptNo).Select(e => new { DeptNo = e.Key, Count = e.Count() }); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(s => s.Comm is not null).Select(s => new {s.EName, s.Comm}).ToList();        
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.Join(grades,
                                ...
                                ((emp, grad) => new
                                {
                                    emp,
                                    grad
                                })).Where(s => s.emp.Sal >= s.grad.Losal && s.emp.Sal <= s.grad.Hisal )
                                .Select(s => new {s.emp.EName,s.grad.Grade});
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(s=>s.DeptNo).Select(e =>new {DeptNo = e.Key , AvgSal = e.Average(s =>s.Sal)}); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > emps.Where(s => e.DeptNo == s.DeptNo).Average(g => g.Sal)).Select(e => e.EName).ToList(); 
